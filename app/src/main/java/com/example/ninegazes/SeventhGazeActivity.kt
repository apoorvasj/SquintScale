...
class SeventhGazeActivity : AppCompatActivity() {
    private var imageBitmap7: Bitmap? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_seventh_gaze)
        val btnTakePicture: Button = findViewById(R.id.btnTakePicture)
        val takePhotoLauncher =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
                if (result.resultCode == RESULT_OK) {
                    imageBitmap7 = result.data?.extras?.get("data") as Bitmap
                }
            }
        ...
        btnNext.setOnClickListener {
            ...
            val byteArray7 = bitmapToByteArray(imageBitmap7!!)
            val intent = Intent(this, EighthGazeActivity::class.java)
