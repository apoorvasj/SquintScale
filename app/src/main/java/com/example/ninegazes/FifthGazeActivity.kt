...
class FifthGazeActivity : AppCompatActivity() {
    private var imageBitmap5: Bitmap? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_fifth_gaze)
        val btnTakePicture: Button = findViewById(R.id.btnTakePicture)
        val takePhotoLauncher =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
                if (result.resultCode == RESULT_OK) {
                    imageBitmap5 = result.data?.extras?.get("data") as Bitmap
                }
            }
        ...
        btnNext.setOnClickListener {
            ...
            val byteArray5 = bitmapToByteArray(imageBitmap5!!)
            val intent = Intent(this, SixthGazeActivity::class.java)
