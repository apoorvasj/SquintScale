...
class NinthGazeActivity : AppCompatActivity() {
    private var imageBitmap9: Bitmap? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_ninth_gaze)
        val btnTakePicture: Button = findViewById(R.id.btnTakePicture)
        val takePhotoLauncher =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
                if (result.resultCode == RESULT_OK) {
                    imageBitmap9 = result.data?.extras?.get("data") as Bitmap
                }
            }
        ...
        btnNext.setOnClickListener {
            ...
            val byteArray9 = bitmapToByteArray(imageBitmap9!!)
            val intent = Intent(this, AllGazesActivity::class.java)
            ...
            intent.putExtra("image9", byteArray9)
